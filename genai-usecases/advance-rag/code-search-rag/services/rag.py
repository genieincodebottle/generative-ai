...
class IndexingPipeline:
    ...
    def index_from_code_string(self, code: str, language: str,
        ...
        logger.debug(f"IndexingPipeline: Indexing code from string - {file_name} ({language})")
        ...
        logger.debug(f"IndexingPipeline: Parsing {language} code...")
        if language == 'python':
        ...
        logger.debug(f"IndexingPipeline: Found {len(chunks)} chunks")
        ...
        if documents:
            logger.debug(f"IndexingPipeline: Adding {len(documents)} documents to vector store...")
            self.vector_store.add_documents(documents)
            logger.debug(f"IndexingPipeline: Documents added successfully")
...
class QueryUnderstanding:
    ...
    def analyze_query(self, query: str) -> Dict[str, Any]:
        ...
        logger.debug(f"QueryUnderstanding: Starting query analysis for: '{query}'")
        try:
            logger.debug(f"QueryUnderstanding: Calling LLM for intent analysis...")
            response = self.llm.invoke(self.intent_prompt.format(query=query))
            logger.debug(f"QueryUnderstanding: LLM response received")
            # Parse JSON from response
            json_match = re.search(r'\{.*\}', response, re.DOTALL)
            if json_match:
                result = json.loads(json_match.group())
                logger.debug(f"QueryUnderstanding: Parsed result: {result}")
                return result
        except Exception as e:
            print(f"[ERROR] Query analysis error: {e}")

        # Default fallback
        logger.debug(f"QueryUnderstanding: Using default fallback analysis")
        return {
            "intent": "how_to",
...
class CodeRetriever:
    ...
    def retrieve(self, query: str, filters: Dict[str, Any] = None) -> List[Document]:
        ...
        logger.debug(f"Stage 0: Starting query understanding...")
        analysis = self.query_understanding.analyze_query(query)
        expanded_query = self.query_understanding.expand_query(query, analysis)
        logger.debug(f"Stage 0: Expanded query: '{expanded_query}'")

        # Stage 1: Vector search - retrieve top-k candidates
        logger.debug(f"Stage 1: Starting vector search (top_k={self.config.top_k_initial})...")
        initial_results = self.vector_store.similarity_search_with_score(
            expanded_query,
            k=self.config.top_k_initial
        )
        logger.debug(f"Stage 1: Found {len(initial_results)} initial results")

        # Stage 2: Metadata filtering
        logger.debug(f"Stage 2: Applying metadata filters...")
        filtered_results = self._apply_filters(initial_results, filters, analysis)
        logger.debug(f"Stage 2: {len(filtered_results)} results after filtering")

        # Stage 3: Reranking
        logger.debug(f"Stage 3: Reranking top {self.config.top_k_rerank} results...")
        reranked_results = self._rerank(query, filtered_results[:self.config.top_k_rerank])
        logger.debug(f"Stage 3: Reranking complete. Returning top {self.config.top_k_final} results")
    ...
    def _rerank(self, query: str, documents: List[Document]) -> List[Document]:
        ...
        if not documents:
            logger.debug(f"_rerank: No documents to rerank")
            return []

        logger.debug(f"_rerank: Reranking {len(documents)} documents...")
        ...
        for i, doc in enumerate(documents[:self.config.top_k_rerank], 1):
            try:
                logger.debug(f"_rerank: Scoring document {i}/{len(documents[:self.config.top_k_rerank])}...")
                ...
                logger.debug(f"_rerank: Document {i} scored: {score}")
            except Exception as e:
        ...
        logger.debug(f"_rerank: Reranking complete")
...
class ResponseGenerator:
    ...
    def generate(self, query: str, documents: List[Document]) -> str:
        ...
        if not documents:
            logger.debug(f"ResponseGenerator: No documents found, returning default message")
            return "I couldn't find relevant code examples for your query. Please try rephrasing or provide more context."

        # Assemble context
        logger.debug(f"ResponseGenerator: Assembling context from documents...")
        code_examples = []
        metadata_info = []
        ...
        # Generate response
        logger.debug(f"ResponseGenerator: Calling LLM to generate final response...")
        ...
        logger.debug(f"ResponseGenerator: Response generated successfully")
...
class RAGCodeSearchSystem:
    ...
    def search(self, query: str, filters: Dict[str, Any] = None) -> str:
        ...
        print(f"\n{'='*80}")
        logger.debug(f"RAGCodeSearchSystem.search() START")
        logger.debug(f"Query: '{query}'")
        logger.debug(f"Filters: {filters}")
        print(f"{'='*80}\n")

        # Retrieve relevant code
        logger.debug(f"Step 1: Retrieving relevant code...")
        documents = self.retriever.retrieve(query, filters)
        logger.debug(f"Step 1: Retrieved {len(documents)} documents")

        # Generate response
        logger.debug(f"Step 2: Generating response...")
        response = self.generator.generate(query, documents)
        logger.debug(f"Step 2: Response generated")

        print(f"\n{'='*80}")
        logger.debug(f"RAGCodeSearchSystem.search() COMPLETE")
        print(f"{'='*80}\n")
    ...
    def search_with_details(self, query: str, filters: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        print(f"\n{'='*80}")
        logger.debug(f"RAGCodeSearchSystem.search_with_details() START")
        logger.debug(f"Query: '{query}'")
        logger.debug(f"Filters: {filters}")
        print(f"{'='*80}\n")

        logger.debug(f"Step 1: Retrieving documents...")
        documents = self.retriever.retrieve(query, filters)
        logger.debug(f"Step 1: Retrieved {len(documents)} documents")
        ...
        logger.debug(f"Step 3: Preparing detailed results...")
        result = {
            'query': query,
            'response': response,
            'documents': [
        ...
        print(f"\n{'='*80}")
        logger.debug(f"RAGCodeSearchSystem.search_with_details() COMPLETE")
        print(f"{'='*80}\n")
