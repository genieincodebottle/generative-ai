...
# LangChain 1.0 emptied `langchain.retrievers` and moved these; the shim tries
# the 1.x location first and falls back to the 0.3 one.
MultiVectorRetriever = compat.multi_vector_retriever()
InMemoryByteStore = compat.in_memory_byte_store()
...
class MultimodalRAGSystem:
    ...
    def __init__(self, google_api_key: str, persist_directory: str = "./chroma_db_v2", 
                 main_model: str = "gemini-pro-latest", vision_model: str = "gemini-flash-latest",
                 embedding_model: str = "models/gemini-embedding-001", temperature: float = 0.1, 
                 max_tokens: int = 8192):
        """
        ...
        # langchain_google_genai reads the key from the environment.
    ...
    def create_summaries_for_retrieval(self, elements: List[Dict]) -> List[str]:
        ...
        for element in elements:
            try:
                ...
                summaries.append(message_text(response))
